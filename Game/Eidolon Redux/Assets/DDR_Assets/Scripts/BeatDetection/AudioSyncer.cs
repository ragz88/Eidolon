...
public class AudioSyncer : MonoBehaviour
{
    ...
    public virtual void OnUpdate()
    {
        ...
        {
            // if audio value between our threshholds
            if ((m_audioValue >= lowerThreshhold &&
                m_audioValue <= upperThreshhold) && 
                (m_previousAudioValue < lowerThreshhold ||
                m_previousAudioValue > upperThreshhold))
            {
                // if minimum beat interval is reached
                if (m_timer > timeStep)
                    OnBeat();
    ...
    public bool specificRange = false;
    public float bias;
    public float upperThreshhold;
